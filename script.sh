...
remote_cmd() { # SSH into the device
...
if [ "$iosversion" = '8.4.1' ]; then
    echo "Doesn't get further than slide to upgrade"
    ...
    echo "I think that'll work"
elif [ "$iosversion" = '7.0.6' ]; then
    echo "Should work?"
elif [ "$iosversion" = '9.3.2' ]; then
    echo "This version gets locked up on the progress bar"
    read -p "would you like to use this version anyway? " response6
    if [[ "$response6" = 'yes' || "$response6" = 'y' ]]; then
        echo "ok"
    else
...
# If we have the files we'll try and boot the phone with it
...
# We yoink the file we need from SSHRD_Script for the iOS 8 ramdisk https://github.com/verygenericname/SSHRD_Script
./img4tool -e -s other/shsh/"${check}".shsh -m IM4M

# Unknown if iPhone 6,2 works at the moment
if [[ "$deviceid" = 'iPhone6,1' || "$deviceid" = 'iPhone6,2' ]]; then

    # Tar the iOS 7.1.2 rootfs file stuff
    if [ "$iosversion" = '8.4.1' ]; then
