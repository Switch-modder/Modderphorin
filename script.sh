...
echo "please install dropbear on cydia from the repo apt.netsirkl64.com"
echo "and then go and install openssh and mterminal as well"
echo "dropbear will enable ssh on ios 10 and openssh enables sftp on ios 10"
...
read -p "Would you like to nuke your partition table?" response1

if [[ "$response1" = 'yes' || "$response1" = 'y' ]]; then

remote_cmd "lwvm init"
sleep 2
./sshpass -p 'alpine' ssh -o StrictHostKeyChecking=no -p2222 root@localhost "sync"
